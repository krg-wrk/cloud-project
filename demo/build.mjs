...
const { people, content, events } = await import(join(here, "../server/dist/data/seed.js"));

const html = readFileSync(join(here, "hub.template.html"), "utf8").replace(
  "__SEED__",
  JSON.stringify({ people, content, events }),
);

const out = join(here, "forecasters-hub.html");
writeFileSync(out, html);
console.log(`${out} — ${people.length} people, ${content.length} pieces, ${events.length} events`);
